...
open class KAppDelegate: OGLAppDelegate, KStreamingProviderSupplier {
    ...
    @objc override func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        KLog("PUSH: registered")
        KPushManager.setPushDeviceToken(deviceToken as Data)
    }
    
    @objc override func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
        KLog("PUSH: fail to register")
    }

    @objc override func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable : Any]) {
